...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname 2019W1-MT1-starter) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #t)))
(require spd/tags)
(require 2htdp/image)
...
(@problem 1)
;;
;; PUT YOUR CS ID HERE ON THIS LINE: Stanley Cheung, future CS major
;;
...
(check-expect (largest 5 5 3) 5)  ; tie for largest
(check-expect (largest 5 3 5) 5)
(check-expect (largest 3 5 5) 5)

(check-expect (largest 1 1 2) 2)  ; tie for smallest
(check-expect (largest 1 2 1) 2)
...
(check-expect (fits? (rectangle 10 20 "solid" "black") 10 20) false)
(check-expect (fits? (rectangle 5 25 "solid" "black") 10 20) false)
(check-expect (fits? (rectangle 15 20 "solid" "black") 10 30) false)
(check-expect (fits? (rectangle 3 5 "solid" "black") 6 10) true)
(check-expect (fits? (rectangle 3 5 "solid" "black") 1 3) false)
...
;; right justify a list of lines to FILL-TO
;; CONSTRAINT: No line may have more than FILL-TO characters
(check-expect (justify LOL0) empty)
(check-expect (justify LOL1) (cons (make-ln 10 "0123456789") empty))
(check-expect (justify LOL3) (cons L0
                                   (cons (make-ln 10 " 123456789")
                                         (cons (make-ln 10 "        89")
                                               empty))))

; (define (justify lol) empty)  ; stub
...
(@htdf justify-line)
(@signature Line -> Line)
;; right justify given line to FILL-TO
