...
;; grade a student's multiple-choice answers
(check-expect (grade empty empty) 0)
(check-expect (grade (list "A" "A" "B") empty) 0)
(check-expect (grade empty (list "A" "D" "B" "C")) 0)
...
; (define (grade losa loca) 0)  ; stub

;;
;; 2 ONE-OF TABLE 
;;
;;         LOSA         empty    (cons String LOSA)
;; LOCA
;;  
;; empty                0 [1]    0 [1]
;;
;; (cons String LOCA)   0 [1]    (cond [(string=? "blank" (first losa))  [2]
;;                                      (grade (rest losa) (rest loca))]
;;                                     [(string=? (first losa) (first loca))
;;                                      (add1 (grade (rest losa) (rest loca)))]
;;                                     [else (sub1 (grade (rest losa)
;;                                                        (rest loca)))])
;;
...
(define (grade losa loca)
  (cond [(or (empty? losa) (empty? loca)) 0]     ; [1]
        [else                                    ; [2] 
         (cond [(string=? "blank" (first losa))
                (grade (rest losa) (rest loca))]
               [(string=? (first losa) (first loca))
                (add1 (grade (rest losa) (rest loca)))]
               [else (sub1 (grade (rest losa) (rest loca)))])]))
