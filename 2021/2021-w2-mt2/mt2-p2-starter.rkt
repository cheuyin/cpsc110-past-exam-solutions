...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname mt2-p2-starter) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #t #t none #f () #f)))
(require spd/tags)
...
;; produce first grant with value that is >= given amount; false if none exists
(check-expect (find-grant-with-value E8 50 500 4000) G17)
(check-expect (find-grant-with-value E7 60 200 5000) false)
(check-expect (find-grant-with-value E3 100 365 50) G15)
(check-expect (find-grant-with-value E3 50 800 9000) G18)
  ...
  (local [(define (fn-for-employee e)
          ...
          (define (fn-for-log log)
            (cond [(empty? log) false]
                  [else
                   (if (>= (fn-for-grant (first log)) amt)
                       (first log)
                       (fn-for-log (rest log)))]))

          (define (fn-for-grant g)
            (cond [(> (grant-vests g) day) 0]
                  [else
                   (* (grant-shares g) (- price (grant-price g)))]))]
