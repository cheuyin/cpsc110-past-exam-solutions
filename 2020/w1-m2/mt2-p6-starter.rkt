...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname mt2-p6-starter) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #t)))
(require spd/tags)
(require 2htdp/image)
...
;; produce tree fractal image
;; CONSTRAINT: Natural is > 0
(check-expect (tree 1) (tr 1))
(check-expect (tree CUTOFF) (tr CUTOFF))
(check-expect (tree 16) (above (beside (rotate 45 (tr (* 0.6 SCALE)))
                                       (rotate -45 (tr (* 0.6 SCALE))))
                               (tr 16)))

(define (tree n)
  (cond [(<= n trivial) (beside (rotate 45 (tr n))
                                (rotate -45 (tr n)))]
        [else
         (local [(define subs (tree (* SCALE n)))]
           (above ...
            (tr n)))])) 
