...
(@signature (listof Integer) Natural -> Boolean)
;; produce true if list is (list N N-1 N-2 ... 1)
(check-expect (descending-from-n? (list) 0) true)
(check-expect (descending-from-n? (list) 1) false)
(check-expect (descending-from-n? (list 1) 1) true)
(check-expect (descending-from-n? (list 4 3 2) 0) false)
(check-expect (descending-from-n? (list 4 3 2) 4) false)
(check-expect (descending-from-n? (list 4 3 2 1) 4) true)
(check-expect (descending-from-n? (list 4 2 2 1) 4) false)

;;        ListOfInteger    empty              (cons Integer ListOfInteger)
;; Natural
;;
;; 0                       (empty? loi) [1]   (empty? loi) [1]
;;
;; (add1 Natural)          false [2]          (and (= (first loi) n)
...
(define (descending-from-n? loi n)
  (cond [(zero? n) (empty? loi)]
        [(empty? loi) false]
        [else
         (and (= (first loi) n)
              (descending-from-n? (rest loi) (sub1 n)))]))
