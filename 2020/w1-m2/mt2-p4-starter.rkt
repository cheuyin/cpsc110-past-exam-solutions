...
(check-expect (fold-person make-person make-marriage identity empty cons empty
                           cons P3)
              P3)

(@template Person (listof Person) Date encapsulated)

(define (fold-person f1 f2 f3 b1 f4 b2 f5 p)
  (local [(define (fn-for-person p)
            (f1 (person-name p)
                (fn-for-date (person-bdate p))
                (fn-for-lom (person-marriages p))))
          ...
          (define (fn-for-marriage m)
            (f2 (fn-for-date (marriage-date m))
                (fn-for-person (marriage-spouse m))
                (fn-for-lop (marriage-children m))))
          ...
          (define (fn-for-lop lop)
            (cond [(empty? lop) b1]
                  [else
                   (f4 (fn-for-person (first lop))
                       (fn-for-lop (rest lop)))]))

          (define (fn-for-lom lom)
            (cond [(empty? lom) b2]
                  [else
                   (f5 (fn-for-marriage (first lom))
                       (fn-for-lom (rest lom)))]))]
    (fn-for-person p)))
