...
(define (next-balloon b)
  (if (and (not (false? b))
           (<= (+ (balloon-r b) GROW-SPEED) MAX-RADIUS))
...
(check-expect (render-balloons (list B1)) MTS)
(check-expect (render-balloons (list B2))
              (place-image (circle 23 "solid" "green")
                           100
                           50
                           MTS))
(check-expect (render-balloons (list B1 B2 B3))
              (place-image (circle 23 "solid" "green")
                           100
                           50
                           (place-image (circle 50 "solid" "green")
                                        20
                                        30
                                        MTS)))
...
(define (render-balloons lob)
  (cond [(empty? lob) MTS]
        [else
         (place-balloon-on (first lob)
                           (render-balloons (rest lob)))]))  



(@htdf place-balloon-on)
(@signature Balloon Image -> Image)
;; place balloon img of radius r, at x, y on the given image
(check-expect (place-balloon-on B1 MTS) MTS)
(check-expect (place-balloon-on B2 MTS) (place-image (circle 23 "solid" "green")
                                                     100
                                                     50
                                                     MTS))

; (define (place-balloon-on b img) empty-image)  ; stub

(@template Balloon)

(define (place-balloon-on b img)
  (cond [(false? b) img]
        [else
