...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname 2020W1-F-P4-solution) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #t)))
(require spd/tags)
...
;; x,y is outside/at/inside left edge
(check-expect (overlapped-by? M1 (- 50 (/ SIZE 2)  1) 150) false)
(check-expect (overlapped-by? M1 (- 50 (/ SIZE 2)  0) 150) true)
(check-expect (overlapped-by? M1 (- 50 (/ SIZE 2) -1) 150) true)

;; above/at/below right edge
(check-expect (overlapped-by? M1 (+ 50 (/ SIZE 2) -1) 150) true)
(check-expect (overlapped-by? M1 (+ 50 (/ SIZE 2)  0) 150) true)
(check-expect (overlapped-by? M1 (+ 50 (/ SIZE 2)  1) 150) false)

;; below/at/above top edge
(check-expect (overlapped-by? M1 50 (- 150 (/ SIZE 2)  1)) false)
(check-expect (overlapped-by? M1 50 (- 150 (/ SIZE 2)  0)) true)
(check-expect (overlapped-by? M1 50 (- 150 (/ SIZE 2) -1)) true)

;; above/at/below top edge
(check-expect (overlapped-by? M1 50 (+ 150 (/ SIZE 2)  1)) false)
(check-expect (overlapped-by? M1 50 (+ 150 (/ SIZE 2)  0)) true)
(check-expect (overlapped-by? M1 50 (+ 150 (/ SIZE 2) -1)) true)
...
(define (overlapped-by? m x y)
  (and (<= (- (mole-x m) (/ SIZE 2))  x  (+ (mole-x m) (/ SIZE 2)))
       (<= (- (mole-y m) (/ SIZE 2))  y  (+ (mole-y m) (/ SIZE 2)))))
