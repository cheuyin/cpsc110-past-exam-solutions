...
;; produce num of boards visited until first nqueens solution; false if unsolved
...
(@template encapsulated try-catch genrec arb-tree)
...
(define (nqueens N)          
  ...
  ;; v-count is Natural; number of boards that have been visited so far
  ;; todo is (listof Board); boards to be visited next
  
  (local [;; Board -> Board or false
          (define (fn-for-bd bd todo v-count)
            (cond [(solved? bd) (add1 v-count)]
                  [else
                   (fn-for-lobd (append (next-boards bd) todo)
                                (add1 v-count))]))

          ;; (listof Board) -> Board or false
          (define (fn-for-lobd todo v-count)
            (cond [(empty? todo) false]
                  [else                 
                   (fn-for-bd (first todo) (rest todo) v-count)]))
