...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname 2020W1-F-P5-solution) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #t)))
(require spd/tags)
...
(define (appear-one-mole lom)
  (find-and-replace (lambda (m) (not (mole-visible m)))
                    appear
                    lom))
...
(define (disappear-mole-at-xy lom x y)
  (find-and-replace (lambda (m) (overlapped-by? m x y))
                    disappear
                    lom))
...
(@htdf find-and-replace)
(@signature (Mole -> Boolean) (Mole -> Mole) (listof Mole) -> (listof Mole))
;; replace the first m in lom for which (find m) produces true with (replace m)
(check-expect (find-and-replace (λ (m) (overlapped-by? m 301 403))
                                disappear
                                (list (make-mole 100 200 (list "x") true)
                                      (make-mole 300 400 (list "y") true)))
              (list (make-mole 100 200 (list "x") true)
...
(check-expect (find-and-replace (λ (m) (not (mole-visible m)))
                                appear
                                (list (make-mole 100 200 (list "x") false)
                                      (make-mole 300 400 (list "y") true)))
              (list (make-mole 100 200 (list "x") true)
                    (make-mole 300 400 (list "y") true)))
...
(check-expect (find-and-replace (λ (m) true) identity empty) empty)
(check-expect (find-and-replace (λ (m) true) identity LOM123456) LOM123456)
