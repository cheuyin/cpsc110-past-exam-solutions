...
(@signature (X -> Boolean) (X -> Y) (listof X) -> (listof Y))
;; find first X in list for which p produces true, and replace with (f X)
(check-expect (find-and-replace negative? add1 empty)
              empty)
(check-expect (find-and-replace negative? add1 (list -1 -2 -3 4 5))
              (list 0 -2 -3 4 5))
(check-expect (find-and-replace positive? add1 (list -1 -3 -5))
              (list -1 -3 -5))
(check-expect (find-and-replace positive? add1 (list -1 -3 -5 6))
              (list -1 -3 -5 7))

(@template (listof X))

(define (find-and-replace p f lst)
...
(define (appear-one-mole lom)
  (find-and-replace (lambda (m) (not (mole-visible m))) appear lom))
...
(define (disappear-mole-at-xy lom x y)
  (find-and-replace (lambda (m) (overlapped-by? m x y)) disappear lom))
