...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname mt2-p3-4-starter) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #t)))
(require spd/tags)
...
;;       Be sure that you use the template and the @template origin properly.
;;
...
;; commented out stub, check-expects, @template tag and function.  You must
...
;; consume TTree and produce number of nodes with 3 falses as children
...
;; produce value from first occurence of key; else false if not found
...
(@template TTree try-catch)

; stops search when node is found

(define (find-val tt k)
  (cond [(false? tt) false]
        [(= k (node-k tt)) (node-v tt)]
        [else
         (local [(define try1 (find-val (node-lef tt) k))]
           (if (not (false? try1))
               try1
               (local [(define try2 (find-val (node-mid tt) k))]
                 (if (not (false? try2))
                     try2
                     (find-val (node-rig tt) k)))))]))
